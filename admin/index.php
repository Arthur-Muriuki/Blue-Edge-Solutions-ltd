...
try {
    $stmt_count = $pdo->query("SELECT COUNT(*) FROM service_requests WHERE status = 'PENDING'");
    $pending_services_count = $stmt_count->fetchColumn();
...
<main style="background-color: #f8fafc; min-height: 80vh; padding: 50px 20px; font-family: 'Segoe UI', Tahoma, Geneva, sans-serif;">
            ...
            <div style="background: white; border-radius: 10px; border: 1px solid #e2e8f0; padding: 30px; box-shadow: 0 4px 10px rgba(0,0,0,0.02); display: flex; flex-direction: column; justify-content: space-between;">
                <div>
                    <div style="display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 20px;">
                        <div style="background: #e0f2fe; color: #0284c7; width: 50px; height: 50px; border-radius: 8px; display: flex; align-items: center; justify-content: center; font-size: 1.5rem;">
                            <i class="ph ph-receipt"></i>
                        </div>
                        <?php if ($pending_services_count > 0): ?>
                            <span style="background: #ef4444; color: white; padding: 4px 10px; border-radius: 12px; font-size: 0.75rem; font-weight: bold;">
                                <?php echo $pending_services_count; ?> Pending
                            </span>
                        <?php endif; ?>
                    </div>
                    <h3 style="color: #002d62; margin: 0 0 10px 0; font-size: 1.3rem;">Service Requests</h3>
                    <p style="color: #64748b; font-size: 0.95rem; line-height: 1.5; margin-bottom: 20px;">
                        Manage monthly cloud support subscriptions & diagnostic bookings.
                    </p>
                </div>
                <a href="admin_subscriptions.php" style="background: #002d62; color: white; text-align: center; text-decoration: none; padding: 12px; border-radius: 6px; font-weight: bold; display: block;">
                    View Requests &rarr;
                </a>
            </div>
